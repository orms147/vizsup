...
### Layout
- **Left = the video preview (hero), ~9:16 vertical phone clip.** It renders the Vietnamese subtitle **in its real style at its real on-screen position over the video frame (WYSIWYG)**, and the subtitle is **draggable** — the user drags it anywhere on the frame to reposition (free placement), updating live. Below it: a transport bar (play/pause, scrub, current-time, volume). The original Chinese hardsub may still be on the frame, so show how an opaque box can sit over it.
...
- A **"Câu đang chọn" panel** for the selected row:
  ...
  - **Số dòng** indicator for this line (1 / 2 / 3 lines as it currently wraps) and a way to insert a **manual line break** in the text, so the user controls whether a line shows as 1 or 2 rows.

### Tab "Kiểu chữ" (global subtitle style — applies to all lines)
- **Mẫu kiểu**: presets dropdown + **Lưu** / **Xóa**.
...
- **Đậm** / **Nghiêng** toggles.
- **Giãn chữ** slider — stretch/compress letter spacing (e.g. −20%…+40%) so a long line can be squeezed onto fewer rows. Pair it with a **Chế độ dòng** control: *Tự động* / *1 dòng* / *2 dòng* (controls wrapping). The goal the user stated: make any line fit comfortably in **1–2 lines** by adjusting spacing/wrap rather than just shrinking the font.
- **Vị trí**: primary method is **drag the subtitle on the video** (left preview); also provide a small **3×3 anchor grid** for quick snap (top/middle/bottom × left/centre/right) and a **"Đặt lại vị trí"** button to clear the free position and return to the grid/margins. A **Lề (cách mép)** slider for fine offset when using the grid.
...
- Backend already supports it: `write_ass` takes a full `style` dict incl. `pos=(nx,ny)` for free drag-placement and `letter_spacing` (\fsp) for the stretch-to-fit control; `style_preview_frame` burns one frame for WYSIWYG.
- Moving the style panel + the burn-preview + drag canvas from Screen 3 to Screen 2; Screen 3's render call already accepts the style (it'll just receive what Screen 2 set).
